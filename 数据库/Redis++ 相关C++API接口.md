...
std::ostream &operator<<(std::ostream &os, const std::unordered_map<T, U>
                         &umap)
{
    os << '[';
    for (auto item : umap)
...
​    // NOTE: The return results of zrangebyscore are ordered, if you save the results  in to `std::unordered_map<std::string, double>`, you'll lose the order.
